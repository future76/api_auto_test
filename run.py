...
from utils.file_utils.file_handle import *
...
def run():
    ...
    # 将http_server.exe和.bat文件复制到allure报告
    copy_file(config.server_path, config.allure_report_path)
    copy_file(config.bat_path, config.allure_report_path)
    #  将allure压缩
    zip_file(config.allure_report_path, config.report_path)
